...
// Класс переименован, чтобы не конфликтовать с уже существующим тестом в CheckinRoutesTest.kt
class EntryManagerCheckInSmokeIT {

    @Test
    fun `happy path ARRIVED`() = testApplication {
        ...
        val initData = emCreateInitData() // корректный initData через helper
    ...
    @Test
    fun `malformed or expired qr returns 400`() = testApplication {
    ...
    @Test
    fun `list not found returns 404`() = testApplication {
        val guestListRepository = EMGuestListRepository()
    ...
    @Test
    fun `entry not found returns 404`() = testApplication {
        val guestListRepository = EMGuestListRepository()
    ...
    @Test
    fun `entry list mismatch returns 400`() = testApplication {
        val guestListRepository = EMGuestListRepository()
    ...
    @Test
    fun `scope mismatch returns 403`() = testApplication {
    ...
    @Test
    fun `missing or invalid init data returns 401`() = testApplication {
        val module = emBaseModule()
        applicationDev { emConfigureApp(module) }
...
private class EMUserRepository : UserRepository {
    private val user = User(id = EM_INTERNAL_USER_ID, telegramId = EM_TELEGRAM_USER_ID, username = "entry_mgr")
...
private fun emCreateInitData(
    ...
    val params =
        linkedMapOf(
            ...
            "auth_date" to Instant.now().epochSecond.toString(), // важно: актуальное время
        )
