...
class RbacPluginTest {
    @Test
    fun `responds 401 when principal missing`() = testApplication {
    ...
    @Test
    fun `responds 403 when role missing`() = testApplication {
        val auditRepo = relaxedAuditRepo()
        val userRepo = InMemoryUserRepository(mapOf(1L to User(id = 1L, telegramId = 1L, username = "one")))
    ...
    @Test
    fun `responds 403 when club scope violated`() = testApplication {
        val auditRepo = relaxedAuditRepo()
        val userRepo = InMemoryUserRepository(mapOf(1L to User(id = 1L, telegramId = 1L, username = "one")))
    ...
    @Test
    fun `responds 200 when access granted`() = testApplication {
        val auditRepo = relaxedAuditRepo()
        val userRepo = InMemoryUserRepository(mapOf(1L to User(id = 1L, telegramId = 1L, username = "one")))
