...
class AppConfigFromEnvTest :
    StringSpec({
        ...
        "missing token fails fast" {
            val error =
                shouldThrow<IllegalStateException> {
                    withEnv(baseEnv + ("TELEGRAM_BOT_TOKEN" to null) + ("RUN_MODE" to null)) {
                        AppConfig.fromEnv()
                    }
                }
        ...
        "loads bot token from TELEGRAM_BOT_TOKEN" {
            ...
            val config =
                withEnv(baseEnv + ("TELEGRAM_BOT_TOKEN" to token) + ("RUN_MODE" to null)) {
                    AppConfig.fromEnv()
                }
        ...
        "supports webhook and polling run modes" {
            ...
            val webhookConfig =
                withEnv(env + ("RUN_MODE" to "webhook")) {
                    AppConfig.fromEnv()
                }
            webhookConfig.runMode shouldBe BotRunMode.WEBHOOK

            val pollingConfig =
                withEnv(env + ("RUN_MODE" to "polling")) {
                    AppConfig.fromEnv()
                }
