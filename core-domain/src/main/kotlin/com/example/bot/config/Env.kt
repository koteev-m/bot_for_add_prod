...
internal fun envInt(name: String, default: Int? = null): Int =
    env(name)?.toIntOrNull() ?: default ?: error("ENV $name is required (int)")

internal fun envLong(name: String, default: Long? = null): Long =
    env(name)?.toLongOrNull() ?: default ?: error("ENV $name is required (long)")

internal fun envBool(name: String, default: Boolean? = null): Boolean =
    env(name)?.let { it.toBooleanStrictOrNull() ?: error("ENV $name must be boolean") }
        ?: default ?: error("ENV $name is required (boolean)")

internal fun envList(name: String, delimiter: Char = ',', trim: Boolean = true): List<String>? =
    env(name)?.split(delimiter)?.map { if (trim) it.trim() else it }?.filter { it.isNotEmpty() }
...
internal fun maskSecret(value: String?, head: Int = 3, tail: Int = 4): String =
    when {
        value == null -> "null"
