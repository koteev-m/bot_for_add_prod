...
class BookingService(
    ...
    suspend fun hold(req: HoldRequest, idemKey: String): Either<BookingError, HoldResponse> = runCatching {
    ...
    suspend fun confirm(req: ConfirmRequest, idemKey: String): Either<BookingError, BookingSummary> = runCatching {
        withContext(Dispatchers.IO) {
            val event =
                readRepo.findEvent(req.clubId, req.eventStartUtc)
                    ?: return@withContext Either.Left(BookingError.NotFound("event not found"))
    ...
    suspend fun cancel(
        ...
        reason: String?,
        idemKey: String,
    ): Either<BookingError, BookingSummary> = runCatching {
        withContext(Dispatchers.IO) {
            val booking =
    ...
    suspend fun markNoShowOverdue(now: Instant): Int = withContext(Dispatchers.IO) {
        // In-memory repository used in tests does not support batch update; this is noop.
        0
    }

    private fun BookingRecord.toSummary(clubId: Long?, totalDeposit: BigDecimal): BookingSummary = BookingSummary(
        id = id,
        clubId = clubId ?: 0L,
        eventId = eventId,
        tableId = tableId,
