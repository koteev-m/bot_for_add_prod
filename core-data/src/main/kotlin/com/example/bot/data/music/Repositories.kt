...
class MusicItemRepositoryImpl(private val db: Database) : MusicItemRepository {
    ...
    override suspend fun listActive(
        ...
        tag: String?,
        q: String?,
    ): List<MusicItemView> = newSuspendedTransaction(Dispatchers.IO, db) {
...
class MusicPlaylistRepositoryImpl(private val db: Database) : MusicPlaylistRepository {
    ...
    override suspend fun getFull(id: Long): PlaylistFullView? = newSuspendedTransaction(Dispatchers.IO, db) {
        val p = MusicPlaylistsTable.select { MusicPlaylistsTable.id eq id }
            .firstOrNull()?.toView() ?: return@newSuspendedTransaction null
        val items = MusicItemsTable.innerJoin(MusicPlaylistItemsTable)
